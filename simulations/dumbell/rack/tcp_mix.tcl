if 0 { 
 *  Elephants and Mice - The tcl script to run a mix of mice and elephants experiments
 *
...
for {set i $N} {$i < [expr $N * ($rep +1)]} {incr i} {
    if {[string compare $sourceAlg "Newreno"] == 0 || [string compare $sourceAlg "DC-TCP-Newreno"] == 0} {
	set tcp($i) [new Agent/TCP/FullTcp/Newreno]
...
set fsize 10000
set x [expr $simulationTime/$rep]
set c 0
for {set j 1} {$j < [expr $rep+1]} {incr j} {
	for {set i 0} {$i < $N} {incr i} {
	   puts "in round $j and starting mice flow [expr $i+($j*$N)] at time [expr 0.0 + ($x * $c)]"
	   $ns at [expr 0.0 + ($x * $c)] "$ns connect $tcp([expr $i+($j*$N)]) $sink([expr $i+($j*$N)])"	
	   $ns at [expr 0.0001 + ($x * $c)] "$ftp([expr $i+($j*$N)]) send $fsize" 
	}
	set c [expr $c + 1] 
}
