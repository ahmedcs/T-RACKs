...
: '
 * run_all_eleph: run all elephants experiment for various parameter settings
...
do
(./run-eleph.sh $simtime $N $qsize $minrto $tcpop $sample $RACK $vmdelay > TCP-$(date +%Y-%m-%d.%H.%M.%S).log ) >& TCP-ERR-$(date +%Y-%m-%d.%H.%M.%S).log
done
done
done
done
