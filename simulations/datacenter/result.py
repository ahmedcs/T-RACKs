...
parser.add_argument("-n","--num",help="total number of flows", type=int, default=10000)
...
timeouts=0
#The number of total Fast retranmists
fastretrs=0
#The number of total RACK retranmists
rackretrs=0
...
if args.input:
	...
	while True:
		...
		if not line:
			break
		if len(line.split())<3 or float(line.split()[1]) > 10.0 :
			unfin = unfin + 1
			continue
		...
		#TCP timeouts
		if(len(line.split()) >= 9):
			fastretr_num=int(line.split()[8])
			fastretrs+=fastretr_num
		
		if(len(line.split()) >= 10):
			rackretr_num=int(line.split()[9])
			rackretrs+=rackretr_num
	...
	fp = open(args.input.replace('.tr','.res'), 'w')

	if args.all:
		print "There are "+str(len(flows))+" flows in total"
		print "There are "+str(args.num - len(flows) - unfin)+" unfinished flows in total"
		print "There are "+str(timeouts)+" TCP timeouts in total"
		print "There are "+str(fastretrs)+" TCP Fast Retransmists in total"
		print "There are "+str(rackretrs)+" TCP RACK Retransmists in total"
		...
		print "99th percentile FCT for "+str(len(median_flows))+" flows in (100Kb,10MB)\n"+str(max(median_flows))
		print "Average FCT for "+str(len(large_flows))+" flows in (10MB,)\n"+str(avg(large_flows))
		print "99th percentile FCT for "+str(len(large_flows))+" flows in (10MB,)\n"+str(max(large_flows))

		
		fp.write('%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s\n' % (str(len(flows)), str(timeouts) , str(avg(flows))
													   , str(len(short_flows)), str(avg(short_flows)), str(max(short_flows)),
													   str(len(median_flows)) , str(avg(median_flows)), str(max(median_flows)),
													   str(len(large_flows)) , str(avg(large_flows)), str(max(large_flows)),
													   str(args.num - len(flows) - unfin), str(fastretrs), str(rackretrs) ) )
		fp.close()
