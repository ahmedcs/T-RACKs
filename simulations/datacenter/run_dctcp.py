...
parser.add_argument("-f", "--file", help="TCL CDF file(required)")
...
meanFlowSize = 1138*1460 #1138*1460
paretoShape = 1.05
...
trace=0 ### 0 no tracing, 1 for trace-all, 2 for trace-all and queue monitor, 3 for all previous and NAM
npersist=args.npersist #### 0 for persistent, 1 for non-persistent (just reset), 2 for non-persistent close then open

spacing=args.spacing
if spacing:
	ns_source = 'source /bigstore/EIED-TCP/pacing-ns-2.35.sh && env'
else:
	ns_source = 'source /bigstore/EIED-TCP/ns-2.35.sh && env'
...
for i in range(len(load_arr)):
		scheme = 'unknown'
		if switchAlg == 'RED' and sourceAlg == 'DCTCP':
			scheme = 'dctcp'
